...
describe("waitlist validation and persistence", () => {
  ...
  it("rejects a non-HTTPS or credential-bearing store URL", () => {
    ...
    expect(
      normalizeStoreUrl("https://name:secret@shop.example.com"),
    ).toBeNull();
    expect(
      validateWaitlistSubmission({
        email: "x@example.com",
...
describe("waitlist welcome delivery", () => {
  ...
  it("leases, sends with an idempotency key, and records provider receipt", async () => {
    mailConfiguration.mockReturnValue({
      configured: true,
      from: "MyMeridian <welcome@mymeridian.io>",
    });
    deliveryFindFirst.mockResolvedValue({
      id: "delivery_1",
      attempts: 0,
      dedupeKey: "waitlist-welcome:signup_1",
    ...
    await expect(
      deliverOneWaitlistEmail(new Date("2026-08-12T00:00:00Z")),
    ).resolves.toBe(true);
    expect(sendEmail).toHaveBeenCalledWith(
      expect.objectContaining({
        to: "owner@example.com",
        idempotencyKey: "waitlist-welcome:signup_1",
        html: expect.stringContaining("favicon-globe.svg"),
      }),
    );
    expect(deliveryUpdateMany).toHaveBeenLastCalledWith(
      expect.objectContaining({
...
describe("newsletter unsubscribe tokens", () => {
  it("verifies only the recipient-specific signed token", () => {
    const env = {
      MERIDIAN_WAITLIST_UNSUBSCRIBE_KEY: "test-only-unsubscribe-key",
