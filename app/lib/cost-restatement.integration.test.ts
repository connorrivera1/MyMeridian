...
describe.skipIf(!TEST_URL)("cost history and restatement, against real Postgres", () => {
  let prisma: any;
  ...
  beforeAll(async () => {
    ...
    ({ recordVariantCost, loadEffectiveCosts } = await import(
      "./cost-history.server"
    ));
    ({
      restateCosts,
    ...
    ({ detectBundlesForShop, rollUpBundleCosts } = await import(
      "./bundles.server"
    ));
    ({ enqueueRecalcJob, leaseNextRecalcJob, drainRecalcQueue } = await import(
      "./recalc-queue.server"
    ));
  ...
  afterAll(async () => {
    if (!prisma) return;
    if (shopId) await prisma.shop.delete({ where: { id: shopId } }).catch(() => {});
  ...
  it("publishes the current cost onto the catalog without disturbing history", async () => {
    const variant = await prisma.variant.findUnique({ where: { id: singleId } });
  ...
  it("freezes each affected month before it moves anything", async () => {
    ...
    const marchSnapshot = snapshots.find((s: any) => s.periodKey === "2026-03");
  ...
  it("re-materialises profit from the restated basis", async () => {
    const march = await prisma.order.findUnique({ where: { id: marchOrderId } });
  ...
  it("proposes multi-pack mappings from the SKU convention, unconfirmed", async () => {
    ...
    const edges = await prisma.bundleComponent.findMany({ where: { shopId } });
    ...
    const caseEdge = edges.find((edge: any) => edge.bundleVariantId === caseId);
  ...
  it("withdraws a derived cost when its mapping is removed", async () => {
    ...
    expect(result.problems.some((p: any) => p.bundleVariantId === caseId)).toBe(
      true,
    );
  ...
  it("fails a job whose handler throws, and keeps it retryable", async () => {
    const { registerRecalcHandler, runRecalcJob, leaseNextRecalcJob: lease } =
    ...
    if (!leased) throw new Error("Expected the queued failure test job to be leased.");

    await runRecalcJob(leased);

    const after = await prisma.recalcJob.findUnique({ where: { id: job.id } });
  ...
  it("seeds an opening cost basis once, and never a second time", async () => {
    const { seedOpeningCostBasis, OPENING_COST_BASIS_AT } = await import(
      "./cost-history.server"
    );
    ...
    expect(rows[0].effectiveAt.getTime()).toBe(OPENING_COST_BASIS_AT.getTime());
  });
  ...
  it("keeps the accounting trail when its job receipt is pruned", async () => {
    ...
    const before = await prisma.periodRestatement.count({ where: { shopId } });
