...
describe("shipping carrier reconciliation", () => {
  it("normalizes ShipStation label cost without retaining tracking or addresses", () => {
    const parsed = parseShipStationLabels({ labels: [{
      label_id: "se-1",
  ...
  it("marks refunded labels as voided", () => {
    expect(parseShipStationLabels({ labels: [{
      label_id: "se-2",
  ...
  it("joins the documented label shipment_id to a shipment order reference", () => {
    ...
    }] }, new Date("2026-08-11T13:00:00Z"), references);
  ...
  it("turns ShopifyQL aggregates into stable order/day observations", () => {
    const parsed = parseShopifyShippingRows({ data: { shopifyqlQuery: {
      parseErrors: [],
      tableData: { rows: [{
  ...
  it("surfaces ShopifyQL parse errors", () => {
    expect(() => parseShopifyShippingRows({ data: { shopifyqlQuery: {
      parseErrors: ["read_reports missing"],
    } } })).toThrow(/read_reports missing/);
  });

  it("matches GIDs and merchant-facing order numbers", () => {
    const orders = [{ id: "o1", shopifyId: "gid://shopify/Order/123", orderNumber: 1001, currency: "USD" }];
    expect(matchOrderReference("gid://shopify/Order/123", orders)?.id).toBe("o1");
  ...
  it("deduplicates equal cross-source totals and exposes real conflicts", () => {
    ...
    expect(selectCarrierTotal([
      { source: ShippingCostSource.SHIPSTATION, amount: "10.00", labelCount: 1, observedAt: at },
      { source: ShippingCostSource.SHOPIFY_SHIPPING, amount: "10.00", labelCount: 1, observedAt: at },
    ])).toEqual({ selected: ShippingCostSource.SHIPSTATION, cents: 1000, conflict: false });

    expect(selectCarrierTotal([
      { source: ShippingCostSource.SHIPSTATION, amount: "10.00", labelCount: 1, observedAt: at },
      { source: ShippingCostSource.SHOPIFY_SHIPPING, amount: "22.00", labelCount: 2, observedAt: at },
    ])).toEqual({ selected: ShippingCostSource.SHOPIFY_SHIPPING, cents: 2200, conflict: true });
  });
});
