...
| Protected Customer Data scope for ShopifyQL-dependent shipping functionality | **WAITING FOR `mymeridian.io` + Shopify approval** | Connor has approved a truthful expanded request but expressly withheld submission until the domain/identity is ready. Request only the fields Shopify requires; disclose that MyMeridian does not query, retain or use name, email, phone or address when it does not consume them. Keep `read_all_orders` correctly supported. |
